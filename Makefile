...
build-darwin-amd64:
		CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 $(GOBUILD) -ldflags "-X main.tagVersion=`git describe --tags`" -o $(BUILD_DIR)/$(BINARY_DARWIN)_amd64 -v ./happynum_cli
build-windows-amd64:
		CGO_ENABLED=0 GOOS=windows GOARCH=amd64 $(GOBUILD) -ldflags "-X main.tagVersion=`git describe --tags`" -o $(BUILD_DIR)/$(BINARY_WINDOWS)_amd64.exe -v ./happynum_cli
